...
def main():
    ...
    def run_tt_petro_inversion(
        ...
        if plot:
            ...
            _=pg.show(inversion_mesh, data=final_mesh_with_model_tt_petro["sat"], cMap=CMAP, logScale=logScale, cMin=C_MIN_SAT, cMax=C_MAX_SAT, ax=axs[1])
            _=pg.show(original_mesh_w_models, data="vp", cMap=CMAP, logScale=logScale, cMin=C_MIN_VP, cMax=C_MAX_VP, ax=axs[2])
            _=pg.show(inversion_mesh, data=final_mesh_with_model_tt_petro["vp"], cMap=CMAP, logScale=logScale, cMin=C_MIN_VP, cMax=C_MAX_VP, ax=axs[3])
