"""Partially joint petrophysical inversion with cross-gradient in the untrusted region example script.

This module runs a partially joint petrophysical inversion with cross-gradient coupling in the untrusted region on a prepared synthetic
crosshole mesh and data. It provides ``run_pjpixg_inversion`` and a ``__main__``
...
def main():
    """Prepare data and run the default partially joint petrophysical + cross-gradient inversion.
    ...
    operators, and runs `run_pjpixg_inversion` with default parameters.
    ...
    path_data_results = path_data_home / "results_pjpi_xg"

    path_figures_home = path_home / "figures"
    path_figures_results = path_figures_home / "figures_pjpi_xg"
    ...
    def run_pjpixg_inversion(
        ...
        if results_dict_path.exists() and not force_recalculate:
            ...
            #* Load final model
            final_mesh_with_model_pjpixg = pg.load(str(path_data_results.joinpath(f"final_model_ttweight_{tt_weight}.bms").absolute()))
            #* Load final response
            final_response_ert_pjpixg = tt.load(str(path_data_results.joinpath(f"final_response_ert_ttweight_{tt_weight}.data").absolute()))
            final_response_tt_pjpixg = tt.load(str(path_data_results.joinpath(f"final_response_tt_ttweight_{tt_weight}.data").absolute()))
            #* Manager set to none
            petrophysical_pjpixg_inversion = None
            ...
            model_transformation_sat_pjpixg = mths.transformation.LogarithmicBarrierTransformationTwoSided(
                lower_barrier = SATURATION_MIN,
                upper_barrier = SATURATION_MAX,
            )

            model_transformation_ert_pjpixg = mths.transformation.LogarithmicBarrierTransformationTwoSided(
                lower_barrier=RESISTIVITY_MIN,
                upper_barrier=RESISTIVITY_MAX,
            )

            model_transformation_tt_pjpixg = mths.transformation.LogarithmicBarrierTransformationTwoSided(
                lower_barrier=1/VELOCITY_MAX,
                upper_barrier=1/VELOCITY_MIN,
            )

            initial_model_pjpixg = mths.ModelInfoMixedGeoPetro(
                ...
                inversion_transformation_petro=model_transformation_sat_pjpixg,
                inversion_transformation_list_geo=[
                    model_transformation_ert_pjpixg,
                    model_transformation_tt_pjpixg,
                ],
            ...
            smoothing_pjpixg_ert = smoothing_operator()
            damping_pjpixg_ert = damping_operator()
            smoothing_pjpixg_tt = smoothing_operator()
            damping_pjpixg_tt = damping_operator()
            ...
            # careful tuning 
            smoothing_pjpixg_ert.weight = regularisation_weights[0][0]
            damping_pjpixg_ert.weight = regularisation_weights[0][1]
            smoothing_pjpixg_tt.weight = regularisation_weights[1][0]
            damping_pjpixg_tt.weight = regularisation_weights[1][1]

            petrophysical_pjpixg_inversion = ppd.GaussNewtonPetrophysicalDecoupled(
                mesh_info=mesh_info_inversion_mesh,
                petrophysical_trust_region=petrophysical_trust_region,
                ...
                initial_model=initial_model_pjpixg,
                single_model_regularisation=[
                    (smoothing_pjpixg_ert, damping_pjpixg_ert),
                    (smoothing_pjpixg_tt, damping_pjpixg_tt),
                    ],
                ...
                inversion_settings={
                    ...
                    "add_xg_for_untrusted_region": True,
                    "xg_weight": 1e6,
                },
            )
            petrophysical_pjpixg_inversion.maximum_update_per_step = max_update_per_step
            petrophysical_pjpixg_inversion.terminate_on_chi2_decrease = 0.01
            petrophysical_pjpixg_inversion.num_solver = "scipy_sparse"
            ...
            petrophysical_pjpixg_inversion.run()

            #* Prepare final model
            final_mesh_with_model_pjpixg = inversion_mesh.copy()
            petrophysical_model_vector_pjpixg = np.ones(inversion_mesh.cellCount()) * np.nan
            petrophysical_model_vector_pjpixg[petrophysical_pjpixg_inversion.petrophysical_trust_region] = petrophysical_pjpixg_inversion.current_model.model_petro
            final_mesh_with_model_pjpixg["sat"] = petrophysical_model_vector_pjpixg
            final_mesh_with_model_pjpixg["res"] = petrophysical_pjpixg_inversion.current_model.model_list_geo[0]
            final_mesh_with_model_pjpixg["vp"] = 1/petrophysical_pjpixg_inversion.current_model.model_list_geo[1]
            ...
            final_response_ert_pjpixg_vector = ert_man_final.fop.response(final_mesh_with_model_pjpixg["res"])
            final_response_ert_pjpixg = ert_data.copy()
            final_response_ert_pjpixg["rhoa"] = final_response_ert_pjpixg_vector
            
            final_response_tt_pjpixg_vector = tt_man_final.fop.response(1/final_mesh_with_model_pjpixg["vp"])
            final_response_tt_pjpixg = tt_data.copy()
            final_response_tt_pjpixg["t"] = final_response_tt_pjpixg_vector

            rel_error_ert_pjpixg = np.linalg.norm(np.array(ert_data["rhoa"])-np.array(final_response_ert_pjpixg["rhoa"]))/np.linalg.norm(np.array(ert_data["rhoa"]))
            chi2_ert_pjpixg = ph.data_to_chi_squared(ert_data, final_response_ert_pjpixg, "rhoa")

            logger.info("ERT: Relative error: %s, for smoothing %s and damping %s", rel_error_ert_pjpixg, regularisation_weights[0][0], regularisation_weights[0][1])
            logger.info("ERT: Chi squared: %s for smoothing %s and damping %s", chi2_ert_pjpixg, regularisation_weights[0][0], regularisation_weights[0][1])

            rel_error_tt_pjpixg = np.linalg.norm(np.array(tt_data["t"])-np.array(final_response_tt_pjpixg["t"]))/np.linalg.norm(np.array(tt_data["t"]))
            chi2_tt_pjpixg = ph.data_to_chi_squared(tt_data, final_response_tt_pjpixg, "t")

            logger.info("TT: Relative error: %s, for smoothing %s and damping %s", rel_error_tt_pjpixg, regularisation_weights[1][0], regularisation_weights[1][1])
            logger.info("TT: Chi squared: %s for smoothing %s and damping %s", chi2_tt_pjpixg, regularisation_weights[1][0], regularisation_weights[1][1])

            data_misfit = [petrophysical_pjpixg_inversion.tracking_dict[iteration]["data_misfit"] for iteration in range(0, petrophysical_pjpixg_inversion.maximum_iterations+1)]
            chi_squared_history = [
                petrophysical_pjpixg_inversion.tracking_dict[iteration]["chi_squared"] for iteration in range(0, petrophysical_pjpixg_inversion.maximum_iterations+1)
            ]
            single_model_regularisation_misfit = [petrophysical_pjpixg_inversion.tracking_dict[iteration]["single_model_regularisation_misfit"] for iteration in range(0, petrophysical_pjpixg_inversion.maximum_iterations+1)]

            if save:
                final_response_ert_pjpixg.save(str(path_data_results.joinpath(f"final_response_ert_ttweight_{tt_weight}.data").absolute()))
                final_response_tt_pjpixg.save(str(path_data_results.joinpath(f"final_response_tt_ttweight_{tt_weight}.data").absolute()))
                final_mesh_with_model_pjpixg.save(
            ...
            result_dict = {
                ...
                "rel_error_ert": rel_error_ert_pjpixg,
                "rel_error_tt": rel_error_tt_pjpixg,
                "final_chi2_ert": chi2_ert_pjpixg,
                "final_chi2_tt": chi2_tt_pjpixg,
                ...
                "iterations": petrophysical_pjpixg_inversion.maximum_iterations,
        ...
        if plot:
            ...
            _=pg.show(inversion_mesh, data=final_mesh_with_model_pjpixg["sat"], cMap=CMAP, logScale=logScale, cMin=C_MIN_SAT, cMax=C_MAX_SAT, ax=axs[1], label="Saturation")
            ...
            _=pg.show(inversion_mesh, data=final_mesh_with_model_pjpixg["res"], cMap=CMAP, logScale=logScale, cMin=C_MIN_RES, cMax=C_MAX_RES, ax=axs[3], label="Resistivity")
            _=pg.show(original_mesh_w_models, data="vp", cMap=CMAP, logScale=logScale, cMin=C_MIN_VP, cMax=C_MAX_VP, ax=axs[4], label="Velocity")
            _=pg.show(inversion_mesh, data=final_mesh_with_model_pjpixg["vp"], cMap=CMAP, logScale=logScale, cMin=C_MIN_VP, cMax=C_MAX_VP, ax=axs[5], label="Velocity")
            ...
            axs[1].set_title(f"Final saturation model PJPI-XG with TT weight {tt_weight}")
            axs[2].set_title("Original model resistivity")
            axs[3].set_title(f"Final resistivity model PJPI-XG with TT weight {tt_weight}")
            axs[4].set_title("Original model velocity")
            axs[5].set_title(f"Final velocity model PJPI-XG with TT weight {tt_weight}")
            if save:
                fig.savefig(
            ...
            residual_container_ert["rhoa"] = np.abs(final_response_ert_pjpixg["rhoa"] - ert_data["rhoa"]) / np.abs(ert_data["rhoa"])
            ...
            fig.suptitle(f"Relative residuals by offset for ERT PJPI-XG with TT weight {tt_weight}")
            if save:
            ...
            residual_container_tt["t"] = np.abs(final_response_tt_pjpixg["t"] - tt_data["t"]) / np.abs(tt_data["t"])
            ...
            ax.set_title(f"Relative residuals TT PJPI-XG with TT weight {tt_weight}")
            if save:
            ...
            return petrophysical_pjpixg_inversion, result_dict

    # Run a default inversion when invoked as a script. XG weight has to be adapted to data weighting.
    for tt_weight in [1, 2, 10]:
        run_pjpixg_inversion(
            tt_weight=tt_weight,
